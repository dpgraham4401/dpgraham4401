...
<template>
  ...
  >
    ...
    >
      <button class="px-4" aria-label="Toggle navigation menu" @click="toggleNav">
        <svg
    ...
    <nav
      ...
      <div class="flex flex-col items-center font-mono font-medium gap-4 sm:flex-row px-4 sm:px-0 mt-16 sm:mt-0"
      >
        <ShipLogo />
        <div v-for="(value, key) in GLOBAL.menu" :key="key">
          <Anchor ariaLabel="foo" :url="value">{{ key }}</Anchor>
        </div>
      </div>
      <div class="flex gap-4 items-center justify-between px-4 sm:px-0">
        <Anchor url={GLOBAL.social.github} ariaLabel="GitHub Repository">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="32"
            height="32"

...
export default {
  ...
  setup() {
    ...
    const quotes = [
      ...
      {
        quote: "All men make mistakes, but a good man yields when he knows his course is wrong, and repairs the evil. The only crime is pride.",
        source: "Sophocles"
      },
      {
...
<template>
  <footer class="mt-16 mb-8">
    <Section class="mb-4">
      <div
        class="vim-border-b transition-transform duration-150 ease-in-out  pb-4 flex flex-col sm:flex-row sm:flex-nowrap gap-8 sm:items-center"
      >
        ...
        <div class="flex flex-start items-start gap-4 w-full">
          <svg
            class="vim-fill transition-transform duration-150 ease-in-out  w-8 h-8 min-h-8 min-w-8 max-h-8 max-w-8 -translate-y-2"
            height="32"
            viewBox="0 0 24 24"
            width="32"
          ...
          <p id="quote" class="vim-text transition-transform duration-150 ease-in-out ">{{ randomQuote.quote }}</p>
        </div>
    ...
    <p class="vim-text transition-transform duration-150 ease-in-out  text-center text-sm font-medium">
